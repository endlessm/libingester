...
class Asset {
    ...
    _setMetadataValue (name, value) {
        const sanitizeMethodName = `sanitize${_.upperFirst(name)}`;
        switch (name) {
        case 'canonicalURI':
        ...
        default:
            if (_.isFunction(this[sanitizeMethodName])) {
                this.metadata[name] = this[sanitizeMethodName](value);
            } else {
    ...
    addMetadataValue (name, value) {
        ...
        const sanitizeMethodName = `sanitize${_.upperFirst(name)}Item`;
        if (_.isFunction(this[sanitizeMethodName])) {
            const sanitized = this[sanitizeMethodName](value);
            this.metadata[name].push(sanitized);
        } else {
    ...
    validateTitle (value) {
        ...
        if (this.metadata.objectType === Asset.ARTICLE_OBJECT_TYPE) {
            ...
            const lengthOptions = { min: 0, max: config.maxTitleLength };
            if (!validator.isLength(_coerce(value), lengthOptions)) {
