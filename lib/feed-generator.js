...
class FeedGenerator {
    ...
    _fetchRssJson (feedUrl, requestOptions = {}) {
        return new Promise((resolve, reject) => {
            const reqOptions = { url: feedUrl, ...utils.requestDefaults, ...requestOptions };
            const req = utils.getUnpromisedRequestWrapper()(reqOptions);
    ...
    async _fetchRssPage (feed, page, items, allLinks) {
        ...
        // FIXME also use this.config.toDate
        const recentEnoughItems = feedJson.items.filter(item => item.pubdate >= config.fromDate);
    ...
    async getUris () {
        const entries = await this._fetchRssPage(this.feedUris, 1, [], new Set());
