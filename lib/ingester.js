 ...
 *
 * This manages the ingestion and controls the script
 * lifecycle. Ingester classes that inherit this base class should
 * create assets and then call addAsset() with them. This will write
 * the assets into a hatch using a HatchWriter.
 *
 * Running the ingester with the run() method will also control the
 * process exit, unless you especify not to do so. After a successful
 * run, it will logs stats and exit the process. The stats contain the
 * number of total ingested assets and the number of failed assets.
 *
 * If there is a large proportion of failed assets, the ingestion is
 * considered "failed". The proportion can be configured with
 * config.hatchFailRateThreshold. If there are failed assets but not
 * more than config.hatchFailRateThreshold, the ingestion is
 * considered "warning". The process will exit with corresponding
 * codes.
 *
 * If an unexpected error happens while ingesting, it will log the
 * error stack and exit the process with failure code.
 *
 */
class Ingester {
    ...
    async addAsset (asset) {
        ...
        await Promise.all(assets.map(a => a.finish()));
