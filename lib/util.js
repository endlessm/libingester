...
function _fetch_rss_page (feed, page, items, max_items, oldest_date) {
    ...
    return _fetch_rss_json(feed_url).then(feed_json => {
        const recent_enough_items = feed_json.items.filter(item => item.pubdate >= oldest_date);
        ...
        if ((is_paginated || is_array) && !done_crawling) {
            if (is_paginated)
                ...
                return _fetch_rss_page(feed, page, new_items, max_items, oldest_date);
        } else {
            return new_items;
        }
    });
 ...
 * @param {string} uri - The URI of the RSS feed (first page)
