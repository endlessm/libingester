 ...
 *
 * @param {number} [maxItems=Infinity] - Maximum number of entries to ingest
 * @param {number} [maxDaysOld=1] - Maximum age of entries to ingest, in days
 * @returns {Object} - Object with parsed settings `max_items` and `oldest_date`
 * @memberof util
 */
function getIngestionLimits(maxItems = Infinity, maxDaysOld = 1) {
    maxDaysOld = process.env.LIBINGESTER_MAX_DAYS_OLD || maxDaysOld;

    const oldestDate = new Date();
    oldestDate.setDate(oldestDate.getDate() - parseInt(maxDaysOld, 10));

    maxItems = process.env.LIBINGESTER_MAX_ITEMS || maxItems;
