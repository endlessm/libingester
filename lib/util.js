...
function fetch_html(uri, encoding) {
    ...
    return fetchWithRetry(options).then(({ body, statusCode }) => {
        if (statusCode >= 300) {
            throw new Error(`Request for page ${encodedUri} got a non-successful ` +
...
const CLEANUP_DEFAULTS = {
    remove: [ 'script', 'noscript', 'style', 'iframe', 'hr', 'br + br', 'center' ],
    removeNoText: ['p'],
