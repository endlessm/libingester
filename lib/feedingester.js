...
class FeedIngester extends URIListIngester {
    ...
    _fetchRssJsonWithRetry (feedUrl, requestOptions, attempt = 1) {
        const maxRetries = this.config.maxRetries;
        const retryBackoffDelay = this.config.retryBackoffDelay;
        ...
        return this._fetchRssJson(feedUrl, requestOptions).catch(err => {
            ...
            if (shouldRetry) {
                ...
                return delayExecution(timeout).then(() => {
                    return this._fetchRssJsonWithRetry(feedUrl, requestOptions, attempt + 1);
                });
            }
