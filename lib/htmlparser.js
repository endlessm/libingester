...
const { pooledRequest } = require('./utils/request');
const Parser = require('./parser');
const ImagesProcessor = require('./imagesprocessor');
const VideosProcessor = require('./videosprocessor');
...
class HtmlParser extends Parser {
    ...
    async fetch () {
        const encodedUri = HtmlParser._encodeUri(this.uri);
    ...
    get bodyProcessors () {
        return [ImagesProcessor, VideosProcessor];
    }

    async processBody (body) {
        const $body = cheerio.load(body);

        for (const BodyProcessorClass of this.bodyProcessors) {
            const processor = new BodyProcessorClass();
            const assets = await processor.process($body);
    ...
    cleanupBody (body) {
        return sanitizeHtml(body, this.cleanupBodyOptions);
    }
    ...
    async parseDocument ($) {
        ...
        body = await this.cleanupBody(body);
