...
class Hatch {
    ...
    _parse_options(options) {
        ...
        const config_options = argv(ARGV_CONFIGFILE_OPTIONS, argv_options);
        if (config_options['config-file']) {
            const cf = config_options['config-file'];
            const file_options = JSON.parse(fs.readFileSync(cf, 'utf8'));
            options = Object.assign(options, file_options);
        }

        return Object.assign(options, argv(ARGV_OPTIONS, argv_options))
    }
