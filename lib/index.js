...
function _ensure_date (value) {
    if (value instanceof Date) {
        if (isNaN(value.getTime())) {
            winston.warn(`Provided an invalid date object`);
            return null;
        }
        return value;
    }
    try {
        ...
        winston.warn(`Could not coerce value into a date: ${value}`);
        return null;
    }
}
...
function _ensure_synopsis (value) {
     if (value === undefined) {
         winston.warn(`Trying to set an empty synopsis! This is most likely an error in the ingester!`);
         return null;
     }
...
class Hatch {
     ...
     */
    finish() {
        winston.info('hatch: waiting for pending downloads...');
        return Promise.all(this._promises)
        .then(() => this._purge_failed_assets())
        .then(() => this._validate_asset_references())
        .then(() => {
            winston.info('hatch: exporting hatch manifest...');
            return this._save_hatch_manifest();
        })
        .then(() => {
            winston.info('hatch: creating hatch tarball...');
        ...
        .then((tarballPath) => {
            winston.info(`hatch: finished, saved to ${tarballPath}`);
        })
        .catch((err) => {
          winston.error(`Could not generate hatch due to uncaught error: ${err.stack}`);
          throw err;
        });
    }
    ...
    copy_to_s3(bucket) {
        ...
        return fs.readdir(this._path).then((files) => {
            ...
            winston.info(`Hatch copied to http://${targetBucket}.s3.amazonaws.com/${hatch_id}`);
        });
    }
}
