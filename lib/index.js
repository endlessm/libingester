...
class Hatch {
    ...
    _purge_failed_assets() {
        ...
        let successful_assets = this._assets.filter(asset => {
            ...
            const dependents = asset.get_dependent_assets();
            ...
            if (has_failed_dependents) {
                // keep track of which siblings we'll have to remove in a later
                // pass
        ...
        // If this hatch had 100% failures, consider it failed and refuse to
        // continue
        if (successful_assets.length === 0 && this._assets.length > 0) {
            throw new Error(`All assets failed! Aborting hatch...`);
        }
