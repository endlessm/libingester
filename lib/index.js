...
['readFile', 'writeFile', 'readdir', 'copy'].forEach((fn) => {
...
function _ensure_synopsis (value) {
    if (value === undefined) {
        logger.warn(`Trying to set an empty synopsis! This is most likely an error in the ingester!`);
        return null;
    }
...
class BaseAsset {
    ...
    get_dependent_assets() {
        ...
        for (const dependent_asset_id of this._get_dependent_asset_ids())
...
class BlogArticle extends BaseAsset {
     ...
     */
    render() {
        const template_path = path.join(__dirname, './assets/blog-article.mst');
        const template = fs.readFileSync(template_path, 'utf8');

        const blog_stylesheet = path.join(__dirname, './assets/blog-stylesheet.scss');
...
class GalleryImageArticle extends BaseAsset {
    ...
    render() {
        const template_path = path.join(__dirname, './assets/gallery-image-article.mst');
        const template = fs.readFileSync(template_path, 'utf8');

        const gallery_image_stylesheet = path.join(__dirname, './assets/gallery-image-stylesheet.scss');
...
class GalleryVideoArticle extends BaseAsset {
    ...
    render() {
        const template_path = path.join(__dirname, './assets/gallery-video-article.mst');
        const template = fs.readFileSync(template_path, 'utf8');

        const gallery_video_stylesheet = path.join(__dirname, './assets/gallery-video-stylesheet.scss');
...
class NewsArticle extends BaseAsset {
    ...
    render() {
        ...
        const template_path = path.join(__dirname, './assets/news-article.mst');
        const template = fs.readFileSync(template_path, 'utf8');

        const news_stylesheet = path.join(__dirname, './assets/news-stylesheet.scss');
...
class Hatch {
    ...
    _validate_asset_references() {
        ...
        this._assets.forEach((asset) => {
            ...
            for (const outgoing_asset_id of asset.get_dependent_assets())
    ...
    copy_to_s3(bucket) {
        ...
        return fs.readdir(this._path).then((files) => {
            ...
            logger.info(`Hatch copied to http://${targetBucket}.s3.amazonaws.com/${hatch_id}`);
        });
    }
}
