...
class Config {
    constructor () {
        this.config = yargs.strict()
            ...
            .group(['hatch-v2', 'live-mode'], 'Developer only options:')
            .argv;

        if (!this.config.fromDate && this.config.maxDaysOld) {
            const fromDate = new Date();
            fromDate.setDate(fromDate.getDate() - this.config.maxDaysOld);
            this.config.fromDate = this.config['from-date'] = fromDate;
        }

        delete this.config.maxDaysOld;
    }
}
